...
export class Selector extends Graphics {
    ...
    public select(cursorPosition: Point) {
        ...
        this._boxOrigin.set(
            Math.min(this._cursorOrigin.x, cursorPosition.x),
            Math.min(this._cursorOrigin.y, cursorPosition.y)
        );
    ...
    private draw(x: number, y: number, width: number, height: number) {
        ...
        this.fill(BOX_BACKGROUND_COLOR);
        this.stroke({ width: BOX_BORDER_WIDTH, color: BOX_BORDER_COLOR });
    }
}
